...
const BettingPanel: React.FC<BettingPanelProps> = ({
    ...
    return (
        <div className="flex flex-col h-full bg-[#0d0d12] border-r border-white/5 p-5 w-[300px] flex-shrink-0 relative">
            ...
            <div className="mb-8">
                <h2 className="text-xl font-orbitron font-bold text-gray-200 mb-4 tracking-wide">Í≤åÏûÑ ?¥Î¶Ñ</h2>
                ...
                <div className="bg-[#151921] rounded-xl p-4 flex items-center justify-between border border-white/5 relative overflow-hidden group">
                    <div className="absolute top-0 left-0 w-1 h-full bg-[#999999]"></div>
                    <span className="text-gray-500 text-[10px] font-bold uppercase tracking-wider">Fixed Bet</span>
                    <div className="flex items-center gap-1 text-[#999999] font-bold font-mono text-lg">
                        <DollarSign size={16} />
                        <span>{betAmount}</span>
                    </div>
                </div>
                ...
                {gameStatus === 'finished' ? (
                    <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2">
                        <div className="text-center mb-2">
                            <span className={`font-orbitron font-bold text-xl ${result === 'win' ? 'text-[#999999] drop-shadow-[0_0_10px_rgba(0,255,153,0.5)]' :
                    ...
                    <button
                        onClick={onReady}
                        disabled={myStatus === 'ready'} // Playing ?ÅÌÉú?êÏÑú??Î≤ÑÌäº?Ä Î≥¥ÏûÑ (ÎπÑÌôú?±Ìôî)
                        className={`w-full font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all uppercase tracking-widest text-lg shadow-lg
                            ${myStatus === 'ready'
                                ...
                                : 'bg-[#999999] hover:bg-[#00cc7a] text-black shadow-[0_0_20px_rgba(0,255,153,0.4)] hover:shadow-[0_0_30px_rgba(0,255,153,0.6)] hover:-translate-y-0.5'
                            }
                        `}
                    >
                        {myStatus === 'ready' ? (
...
const PlayerCard: React.FC<{
    ...
    <div className={`relative p-4 rounded-2xl border transition-all duration-300 group
        ${isMe ? 'bg-[#151921] border-white/10 hover:border-[#999999]/30' : 'bg-[#151921] border-white/5'}
        ${result === 'win' ? 'ring-2 ring-[#999999] shadow-[0_0_20px_rgba(0,255,153,0.15)]' :
            result === 'lose' ? 'opacity-50 grayscale' : ''}
    `}>
        <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-full overflow-hidden border-2 p-0.5 ${isMe ? 'border-[#999999]' : 'border-gray-700'}`}>
            ...
            <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                    ...
                    {isMe && <span className="text-[9px] bg-[#999999] text-black px-1.5 py-0.5 rounded font-bold">ME</span>}
                    {status === 'ready' && <CheckCircle size={14} className="text-[#999999]" />}
                </div>
